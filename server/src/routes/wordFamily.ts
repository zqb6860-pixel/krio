...
wordFamilyRouter.get('/:word', async (req, res: Response) => {
  try {
...
wordFamilyRouter.get('/root/:root', async (req, res: Response) => {
  try {
    ...
    if (roots.length === 0) {
      return res.status(404).json({ error: '未找到该词根相关单词' });
    }
...
// GET /api/word-family/explore - 浏览常用词根列表
wordFamilyRouter.get('/explore/roots', async (_req, res: Response) => {
  try {
    // 获取所有词根并按出现频率排序
    const roots = await prisma.wordRoot.findMany({
      select: {
